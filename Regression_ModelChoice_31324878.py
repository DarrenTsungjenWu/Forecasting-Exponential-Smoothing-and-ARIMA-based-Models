...
#Model 1
err_train1 = FTSE_train - F; MSE_train1 = sum(err_train1**2)*1.0/len(F)
err_test1 = FTSE_test - E; MSE_test1 = sum(err_test1**2)*1.0/len(E)
#Model 2
err_train2 = FTSE_train - FF; MSE_train2 = sum(err_train2**2)*1.0/len(FF)
err_test2 = FTSE_test - EE; MSE_test2 = sum(err_test2**2)*1.0/len(EE)

#Model 3
err_train3 = FTSE_train - FFF; MSE_train3 = sum(err_train3**2)*1.0/len(FFF)
err_test3 = FTSE_test - EEE; MSE_test3 = sum(err_test3**2)*1.0/len(EEE)
#Show
print('Model1: The MSE of training and test set are: {0}, {1}'.format(MSE_train1, MSE_test1))
print('Model2: The MSE of training and test set are: {0}, {1}'.format(MSE_train2, MSE_test2))
print('Model3: The MSE of training and test set are: {0}, {1}'.format(MSE_train3, MSE_test3))

ci_low = EEE - 1.96*MSE_train3
ci_up = EEE + 1.96*MSE_train3
###############################

# Plotting the graphs
from matplotlib.legend_handler import HandlerLine2D
line1, = plt.plot(F, color='red', label='Fitted Value of Training Set')
...
plt.title('FTSE regression forecast')
plt.show()

from matplotlib.legend_handler import HandlerLine2D
line1, = plt.plot(FF, color='red', label='Fitted Value of Training Set')
line2, = plt.plot(FTSE, color='black', label='Original data')
#line3, = plt.plot(ci_low, color='blue', label='95% Confidence Interval Upper Bound')
#line4, = plt.plot(ci_low, color='orange', label='95% Confidence Interval Lower Bound')
plt.legend(handler_map={line1: HandlerLine2D(numpoints=4)})
...
from matplotlib.legend_handler import HandlerLine2D
line1, = plt.plot(FFF, color='red', label='Fitted Value of Training Set')
line2, = plt.plot(FTSE, color='black', label='Original data')
#line3, = plt.plot(ci_low, color='blue', label='95% Confidence Interval Upper Bound')
#line4, = plt.plot(ci_low, color='orange', label='95% Confidence Interval Lower Bound')
plt.legend(handler_map={line1: HandlerLine2D(numpoints=4)})
